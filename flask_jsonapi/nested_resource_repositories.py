...
class NestedResourceRepositoryViewSet(resource_repositories.ResourceRepositoryViewSet):
    nested_schema = descriptors.NotImplementedProperty('schema')
    ...
    def as_list_view(self, view_name):
        return self.decorate(
            NestedResourceRepositoryListView.as_view(view_name, filter_schema=self.filter_schema, **self.get_nested_schema_kwargs())
        )

    def get_nested_schema_kwargs(self):
