    ...
    def order_by(self, *args):
        ...
        for idx, arg in enumerate(args):
            ...
            for token in arg.split('__'):
                column, join = pomidor(self._raw_columns, token)
                if join:
                    q = q.join(column)
...
def pomidor(entities, token):
    for entity in entities:
        for column in entity.columns:
            if column.key == token:
                return column, False
        for column in entity.foreign_keys:
            if column.column.table.name == token:
                return column.column, True
            try:
                return _entity_descriptor(column.column.table, token), True
            except exc.InvalidRequestError:
                pass
    raise Exception(token, entities)
